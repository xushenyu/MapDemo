package com.chengmao.mapdemo.luban;
...
public class Luban implements Handler.Callback {
	...
	private File getImageCacheFile(Context context, String suffix) {
		...
		String cacheBuilder = mTargetDir + "/" + System.currentTimeMillis() + (int) (Math.random() * 1000)
	...
	/**
	 * Returns a directory with a default name in the private cache directory of the
	 * application to use to store retrieved audio.
	 *
	 * @param context
	 *            A context.
	 *
	...
	/**
	 * Returns a directory with the given name in the private cache directory of the
	 * application to use to store retrieved media and thumbnails.
	 *
	 * @param context
	 *            A context.
	 * @param cacheName
	...
	private File getImageCacheDir(Context context, String cacheName) {
		...
		if (cacheDir != null) {
			...
			if (!result.mkdirs() && (!result.exists() || !result.isDirectory())) {
				// File wasn't able to create a directory, or the result exists but not a
				// directory
				return null;
			}
			return result;
